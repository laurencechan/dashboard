...
//                        });
        ...
        // {
        //   var g = this;
        // //修改 表格对齐问题 start, change by Siven
        //   if(jQuery(g.element).find(".l-grid-body")[1].scrollHeight > jQuery(g.element).find(".l-grid-body")[1].clientHeight){
        //                   jQuery(g.element).find(".l-grid-header-inner")[1].style.overflow = 'scroll'
        //     setTimeout(function(){jQuery(g.element).find(".l-grid-header-inner")[1].style.width = jQuery(g.element).find(".l-grid-body")[1].clientWidth + 'px'},50)
        //                 jQuery(g.element).find(".l-grid-header-inner")[1].style.overflow = 'hidden'
        //   }
        //   else{
        //     jQuery(g.element).find(".l-grid-header-inner")[1].style.width = '100%'
        //   }
        //   if(jQuery(g.element).find(".l-grid-detailpanel-inner")){
        //     jQuery(g.element).find(".l-grid-detailpanel-inner").width(jQuery(g.element).find(".l-grid-header-inner")[1].style.width)
        //   }
        //   jQuery(g.element).find(".l-grid-body:eq(1) tr:eq(0) td").each(function(index){
        //     $(this).width(jQuery(g.element).find(".l-grid-header:eq(1) td").eq(index).width())
        //   })
        //   //end
        //     var inner = g.gridbody.find(".l-grid-body-inner:first");
        //     if (!inner.length) return false;
            ...
            var ajaxOptions = {
                ...
                success: function (data)
                {
                    ...
                    if(len){
                        ...
                        $(g.gridbody).css({'background':'url(../skin/blue/images/public/data_null.png) center no-repeat'})
                    }
                    // -end
                },
                complete: function ()
        ...
        */
        ...
        _fillGridBody: function (data, frozen)
        {
            var g = this, p = this.options;
            //加载数据 
            var gridhtmlarr = ['<div class="l-grid-body-inner" ><table class="l-grid-body-table" cellpadding=0 cellspacing=0 style="width:100%;"><tbody>'];
       ...
       _getCellHtml: function (rowdata, column)
        ...
        _setEvent: function ()
        {
            ...
            $('span.pcontrol :text', g.toolbar).on('keypress',function(e){
                if(e.keyCode == "13"){
                    g.changePage('input');
                }
            });
        ...
        _onResize: function ()
        {
            ...
            if (p.height && p.height != 'auto')
            {
                ...
                $(".l-grid-body-inner").css("width","100%");
                $(".l-grid-body-inner table tr").css("width","100%");
                var tabw = $(".l-grid-body-inner").width();
                $(".l-grid-header-inner").css("width",tabw+"px");
                $(".l-grid-header-table,.l-grid-row-cell-inner").css("width","100%");
