...
from  nova.db.sqlalchemy import test_db_connection as con
...
class ServersController(wsgi.Controller):
    ...
    def create(self, req, body):
        ...
        if server_dict.get('imageRef'):
            image_uuid = self._image_from_req_data(server_dict, create_kwargs)
            LOG.info("image_uuid=========%s"%image_uuid)
        elif server_dict.get('imagegroupRef'):
           image_group_uuid = self._imagegroup_from_req_data(server_dict,create_kwargs)
           image_uuid = get_image_uuid_from_imagegroup(image_group_uuid)
           LOG.info("image_uuid==============%s"%image_uuid)
    ...
    def delete(self, req, id):
        ...
        try:
            ...
            common.raise_http_conflict_for_instance_invalid_state(state_error,
                    'delete', id)
    def _imagegroup_from_req_data(self,server_dict,create_kwargs):
        """Get imagegroup data from request or raise appropriate execptions"""
        imagegroup_href = server_dict.get('imagegroupRef')
        if not imagegroup_href:
            msg = _("Missing imagegropuRef attribute")
            raise exc.HTTPBadRequest(explanation=msg)
        else:
             return imagegroup_href
