...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def _get_cpu_info(self):
        ...
        return cpu_info
    #add by xieweijie on 2017-06-15
    def _get_compute_info(self):
        compute_info=dict()
        try:
            system_xml=self._host.get_connection().getSysinfo()
            cfgsys=vconfig.LibvirtConfigDomainSysinfo()
            cfgsys.parse_str(system_xml)
            basecap=cfgsys.baseboard
            cpucap=cfgsys.processor
            netinfo=self._host.list_net_devices()
            compute_info['netcardnum']=self._net_card_filter(netinfo)
		#compute_name=self._host.list_system_devices()
		#system_info = self._get_system_info(compute_name[0])	
            compute_info['product']=basecap.product
            compute_info['manufacturer']=basecap.manufacturer
            compute_info['CpuProduct']=cpucap.cpu_product
        except libvirt.libvirtError as ex:
            error_code = ex.get_error_code()
            if error_code == libvirt.VIR_ERR_NO_SUPPORT:
    ...
    def _net_card_filter(self,netcard):
        netcardnum=0
        for net in netcard:
            if "lo" in net:
                continue
                ...
                netcardnum+=1
        return netcardnum
    def _get_system_info(self,devname):
        system=self._host.device_lookup_by_name(devname)
        xmlstr = system.XMLDesc(0)
        cfgsys=vconfig.LibvirtConfigNodeDevice()
        cfgsys.parse_str(xmlstr)
        syscap=cfgsys.sys_capability
        systeminfo=[]
        systeminfo.append(syscap.product)
        hdwarecap=syscap.hardware_cap
        systeminfo.append(hdwarecap.vendor)
        return systeminfo
    #end
    ...
    def get_available_resource(self, nodename):
        ...
        #add by xieweijie on  2017-06-15
        data["compute_info"] = jsonutils.dumps(self._get_compute_info())
        #end
