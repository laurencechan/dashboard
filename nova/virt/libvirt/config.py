...
#add by xieweijie on 2017-06-16
#for syetem info
class LibvirtConfigDomainSysinfo(LibvirtConfigObject):
    def __init__(self,**kwargs):
        super(LibvirtConfigDomainSysinfo,self).__init__(root_name="sysinfo",**kwargs)
        self.baseboard=None
        self.processor = None
    def parse_dom(self, xmldoc):
        super(LibvirtConfigDomainSysinfo,self).parse_dom(xmldoc)
        for c in xmldoc.getchildren():
            if c.tag=="baseBoard":
                basecap=LibvirtConfigDomainBaseboard()
                basecap.parse_dom(c)
                self.baseboard=basecap
            elif c.tag=="processor":
                procap=LibvirtConfigDomainProcessor()
                procap.parse_dom(c)
                self.processor=procap
class LibvirtConfigDomainBaseboard(LibvirtConfigObject):
    def __init__(self,**kwargs):
        super(LibvirtConfigDomainBaseboard,self).__init__(root_name="baseBoard",**kwargs)
        self.manufacturer=None
        self.product =None
    def parse_dom(self, xmldoc):
        super(LibvirtConfigDomainBaseboard, self).parse_dom(xmldoc)
	for c in xmldoc.getchildren():
            if c.tag == "entry" and c.get("name") =='manufacturer':
                self.manufacturer=c.text
            elif c.tag == "entry" and c.get("name") == 'product':
                self.product=c.text
class LibvirtConfigDomainProcessor(LibvirtConfigObject):
    def __init__(self,**kwargs):
        super(LibvirtConfigDomainProcessor,self).__init__(root_name="processor",**kwargs)
        self.cpu_product = None
    def parse_dom(self, xmldoc):
        super(LibvirtConfigDomainProcessor, self).parse_dom(xmldoc)
	for c in xmldoc.getchildren():
            if c.tag == "entry" and c.get("name")== 'version':
                self.cpu_product=c.text
#end
class LibvirtConfigNodeDevice(LibvirtConfigObject):
    ...
    def __init__(self, **kwargs):
        ...
        #add by xieweijie on 2017-06-16 for syetem cap
        self.sys_capability=None
    ...
    def parse_dom(self, xmldoc):
        ...
        for c in xmldoc.getchildren():
            ...
            #add by xieweijie on 2017-06-16
            elif c.tag == "capability" and c.get("type") == 'system':
                syscap = LibvirtConfigNodeDeviceSystemCap()
                syscap.parse_dom(c)
                self.sys_capability=syscap
#add by xieweijie on 2017-06-16
#for libvirt config for node device of computer
class LibvirtConfigNodeDeviceSystemCap(LibvirtConfigObject):
        def __init__(self,**kwargs):
            super(LibvirtConfigNodeDeviceSystemCap,self).__init__(
                root_name="capability",**kwargs)
            self.product=None
            self.hardware_cap=None
        def parse_dom(self,xmldoc):
            super(LibvirtConfigNodeDeviceSystemCap,self).parse_dom(xmldoc)
            for c in xmldoc.getchildren():
                if c.tag == "product":
                    self.product=c.text
                elif c.tag == "hardware":
                    hardware_cap=LibvirtConfigNodeDeviceSystemSubHardware()
                    hardware_cap.parse_dom(c)
                    self.hardware_cap=hardware_cap

class LibvirtConfigNodeDeviceSystemSubHardware(LibvirtConfigObject):
    def __init__(self,**kwargs):
        super(LibvirtConfigNodeDeviceSystemSubHardware,self).__init__(
            root_name="hardware",**kwargs
        )
        self.vendor = None
    def parse_dom(self, xmldoc):
        super(LibvirtConfigNodeDeviceSystemSubHardware,self).parse_dom(xmldoc)
        for c in  xmldoc.getchildren():
            if c.tag == "vendor":
                self.vendor=c.text
#end
