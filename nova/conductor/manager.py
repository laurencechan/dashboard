...
class ComputeTaskManager(base.Base):
    ...
    def build_instances(self, context, instances, image, filter_properties,
        ...
        try:
            ...
            return
        LOG.info("===============================instances:%s"%instances)
            ...
            if num_attempts <= 1:
                ...
                try:
                    ...
                    if inst_mapping:
                        inst_mapping.destroy()
                    return
            LOG.info("===================context:%s==========instance:%s=======host:%s==========image:%s"%(context,instance,host['host'],image))
            LOG.info("===================request_spec:%s=========admin_password:%s==============filter_properties:%s"%(request_spec,local_filter_propes,admin_password))
            LOG.info("===================injected_files:%s========request_networks:%s======security_groups:%s"%(innjected_files,request_networks,security_groups))
            LOG.info("===================block_device_mapping:%s======node:%s=========limits:%s"%(bdms,host['nodename'],host['limits']))
