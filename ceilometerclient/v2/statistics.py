...
class StatisticsManager(base.Manager):
    ...
    def single_statistic(self, meter_name, q=None, period=None, computes=None):
        p = '?period=%s' % period if period else ''
        if computes:
            p = '%s&computes=%s' % (p, computes)
        return self.client.get('/v2/meters/' + meter_name + '/statisticsByHost' + p).json()


    def get_statistic_instance(self, meter_name, instance_id, period=60):
        p = '?instance_uuid=%s&period=%s' % (instance_id, period)
        return self.client.get('/v2/meters/' + meter_name + '/statisticsByInstance' + p).json()
